...
from network.mymodel import MultiModalDetection, SimilarityModule, MultiReconstructNet
from utils.loss import FocalCenterDistLoss, ClusteringAnomalyScore
import copy, math
...
def main():
    ...
    log_template_df = pd.read_csv('hipster_log_template.csv')
    step = 10
    interval = 60
    log_seq_len = 512
    epoch_num = 200
    ...
    service_list = ['paymentservice-9cdb6588f-554sm',
    ...
    service = 'checkoutservice-578fcf4766-9csqn'

    dataset = SavedDataset(f"data/15_15_60/{service}/")
    ...
    test_loader = DataLoader(dataset=anomaly_test_set + normal_test_set, batch_size = 4, shuffle=False, collate_fn = data_fliter,  drop_last=True, num_workers=4)
    ...
    optim_task_AD = torch.optim.Adam(
    ...
    for epoch in range(epoch_num):
        ...
        for step, (batch_m, batch_t, batch_l, batch_l_seq, batch_label, ts) in enumerate(train_loader):
            ...
            occ_loss = FocalCenterDistLoss([m, l, t, correlation], 
        ...
        print('*******************************************************************')
        print("Epoch {:04d} | f1: {:.4f} | pre: {:.4f} | recall: {:.4f} | threshold: {:.4f}".format(epoch+1, f1, p, r, th))
        print('*******************************************************************')


    print('*******************************************************************')
    print("best f1: {:.4f} | best pre: {:.4f} | best recall: {:.4f} | TP: {:04d} | FP: {:04d} | FN: {:04d} | best weights: {:.4f}, {:.4f} | best threshold: {:.4f}".format(best_f1, best_p, best_r, best_tp, best_fp, best_fn, best_w1, best_w2, best_th))
    print('*******************************************************************')
